...
#pragma warning (pop)
	...
	*/
	binmap_t::~binmap_t()
	{
		if (cell_)
		{
			u32 const idx = allocator->to_idx(cell_);
			allocator->ideallocate(idx);
		}
	}
	...
	*/
	void binmap_t::pack_cells(ref_t* href)
	{
		...
		if (cell_[ref].is_left_ref_ || cell_[ref].is_right_ref_ || cell_[ref].left_.bitmap_ != cell_[ref].right_.bitmap_)
		{
			return;
		}
